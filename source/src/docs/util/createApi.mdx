...
> Support for Vite is introduced in v1.6.1.
...
const api = createApi({
    ...
    // mock is only used when `REACT_APP_MOCK` or `VITE_MOCK`
...
// calling either implementation or mock depends on `REACT_APP_MOCK` or `VITE_MOCK`
...
> **Important**
  ...
  and exclude the mock by bundler config or treeshaking. See [explanation below](#s-purging-mock-in-production-build).
...
## Using factory callback for mock
...
## Purging mock in production build

When treeshaking is enabled using WebPack or Vite, the simplest way to purge mock from production build
is to add condition with environment variable.
...
Another way is to exclude the entire mock module by bundler configuration:
