...
<ImportHint
    name="ZetaEventContainer"
    module="zeta-dom/events"
    require="EventContainer"
    global="zeta.EventContainer"
    expanded
/>

<MemberList
    i="ZetaEventContainer"
    ip={['Target', 'element', 'context', 'event', 'autoDestroy', 'normalizeTouchEvents', 'captureDOMEvents']}
    im={['getContexts()', 'add()', 'delete()', 'destroy()', 'emit()', 'emitAsync()', 'tap()', 'flushEvents()', 'has()']}
/>
...
### `ZetaEventContainer.Target` <Badge.ReadOnly /> <Badge.Version />

Gets the event target class associated with this container.

The class will have the `on` method to register event handlers to this container.
...
### `ZetaEventContainer.has()` <Badge.Version />
...
- **0.6.4** `ZetaEventContainer.has()`
- **0.6.6** `ZetaEventContainer.Target`
</VersionTimeline>
