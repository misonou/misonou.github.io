...
The `App` object is returned by `brew` function and is passed to initialization callback.
See [Creating app instance](/docs/app/create-app).
...
The tab is about to hide or unload, for example when navigating to other site or switching to another tab in mobile devices.

#### Event properties

<dl>
    #### `persisted`
    A flag that indicates if the current page is persisted when will be resumed,
    typically when user switches back to current tab in mobile devices.
