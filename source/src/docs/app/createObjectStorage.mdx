...
In version 0.7.4 or before, similar to native JSON serialization the following limitations also apply:

- Big integers are not allowed
- `NaN` and `Infinity` are treated as `null`; whereas `-0` is treated as `0`
- Object property having value `undefined` is ignored
- Array member having value `undefined` and array hole (or "empty slot") are treated as `null`
