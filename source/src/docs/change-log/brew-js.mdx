...
- <kbd>fix</kbd> discard remaining segments when an optional parameter is unmatched

#### Utilities

- <kbd>fix</kbd> reset CSS classes and emit [`flyouthide` event](/docs/dom/closeFlyout#s-flyouthide-event)
  when flyout element is detached without calling [`closeFlyout`](:)

## v0.7.9 (2026/06/03)

#### IdleTimeout

- Add `elapsed` property for [`idle` event](/docs/app/useIdleTimeout#s-idle-event)
- Improvement on timer
  - timer now starts after app ready, avoiding `idle` event during app setup
  - timer now has sub-second precision instead of 10 second polling
- <kbd>fix</kbd> `idle` event may be triggered early or immediately after
  - previous `idle` event is handled with a promise
  - opening in new tab when `crossFrame` option is enabled

## v0.7.8 (2026/05/18)

- Add [`brew.disposableWith`](:)
- Improved error handling during app initialization
  - [`brew()`](:) will always return app object instead of throwing
  - [`app.readyState`](:) will turn `error`
    - instead of `init` when extension throws during initialization
    - instead of `ready` when required extension dependency is missing
  - Multiple initialization errors are now all reported to DOM and console
- <kbd>fix</kbd> unable to infer app type when importing `brew-js/core` or `brew-js/app` <Badge.TS/>

## v0.7.7 (2026/05/08)

#### Core

- <kbd>fix</kbd> initialize extension before end of app initialization callback unless missing dependencies are not optional

#### Scrollable

- <kbd>fix</kbd> issue with nested scrollable elements introduced in last version

## v0.7.6 (2026/05/02)

#### Core

- Animate `filter` CSS property when `fade-in` is specified in `animate-in` directive with CSS variable `--animate-fade-filter`
- <kbd>fix</kbd> [`createApiClient`](:) should call `error` trap for all non-abortion error with the original error as `cause`

#### Router

- Add [`HistoryStorageContainer.registerType`](:) for allowing custom type in history storage, including
  [`app.sessionStorage`](:) and [`app.cache`](:)

#### I18n

- <kbd>fix</kbd> missing router in extension dependency

#### Scrollable

- <kbd>fix</kbd> ~~unwanted scrolling due to scroll padding incorrectly applied for elements~~ (further fixed in v0.7.7)

## v0.7.5 (2026/04/13)

Fixes and improvements to object storage:

- support big integers
- correctly persist the following:
  - numeric values `NaN`, `Infinity` and `-0`
  - object property or array member which value is `undefined`
  - array holes (or "empty slots")
- correctly persist and revive custom type object objects when `toJSON` method returns primitive values
- references to a custom type object could be mutated when target object is overwritten using [`ObjectStorage.set`](:)
- references to a primitive object (`Boolean`, `Number` and `String`) will be coerced to its represented primitive value by default

## v0.7.4 (2026/03/26)

- <kbd>fix</kbd> ensure component is initialized when calling [`getDirectiveComponent`](:) of other element during component initialization
- <kbd>fix</kbd> object storage failed to persist object reference and recover custom type when object contains `toJSON` method

## v0.7.3 (2025/12/12)

#### Scrollable

- [`app.useScrollable`](:) now accept `handle` as default option
- <kbd>fix</kbd> scrollable was disabled after modal change

#### Others

- Only polyfill waterpipe template function when needed

## v0.7.1 (2025/01/17)

#### Scrollable

- <kbd>fix</kbd> unable to import `jq-scrollable` when using Rollup

#### Others

- Add `closeOnNavigate`, `popover`, `preventLeave` and `preventNavigation` options for [`openFlyout`](:)
- <kbd>fix</kbd> [`closeFlyout`](:) should resolve to `undefined`
- <kbd>fix</kbd> [`openFlyout`](:) should reject when flyout element is detached
- <kbd>fix</kbd> unable to focus content in modal flyout

## v0.7.0 (2024/12/24)

#### Core

- Trigger `error` event when app failed to initialize

#### HTML router

- Match and mount active elements in `beforepageload` event instead of `navigate` event
- Recognize dynamically inserted `[match-path]` elements after DOM ready

#### Scrollable

- Default mode change to `auto` when `[scrollable]` directive did not specify one of the mode (previously defaults to `content`)
- <kbd>fix</kbd> flickering when scrolling is initiated by dragging content

#### Others

- Add [`createApiClient`](:)
- Request lock cancellation with [`NavigationCancellationRequest`](:) on navigation
- <kbd>fix</kbd> correct type definition of `destroy` event of `ComponentContext` <Badge.TS />
...
- Add `pushstate` event
- Add [`app.navigationType`](:)

#### Scrollable

- Improvement on restoring scroll position when content is loaded asynchonously
- Restore scroll position on reload and between snapshot
- Accept value for `[persist-scroll]` directive

#### Others

- Add `[method-return]` directive
- Support `head` and `patch` request in [`api`](:)
- Export [`resetVar`](:) and [`getVarScope`](:) in UMD distribution

## v0.6.15 (2024/11/08)

#### Router

- Navigating method like [`app.navigate`](:) now does not trigger `unhandledrejection`  event in normal cases, except when
  - returned promises are chained without rejection handler; or
  - excessive navigation is stopped by router
- <kbd>fix</kbd> route parameters should be reverted when navigation is cancelled or rejected

#### I18n

- Add `cookieOptions` option [[link]](/docs/app/useI18n)
- Returns promise from [`app.setLanguage`](:) indicating whether operation is successful
- <kbd>fix</kbd> update language in sync with router navigation lifecycle
- <kbd>fix</kbd> [`app.detectLanguage`](:) returned uncanonicalized value
- <kbd>fix</kbd> app may be initialized with unlisted language when falling back to default language

#### Scrollable

- <kbd>fix</kbd> unable to restore scroll position

#### Animations

- <kbd>fix</kbd> [`animateOut`](:) erroneously reset states for unanimated elements
