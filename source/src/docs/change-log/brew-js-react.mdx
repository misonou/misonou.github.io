...
- <kbd>fix</kbd> determine final path from multiple view containers
- <kbd>fix</kbd> unmount current view when [`renderView`](:) receives new view components

## v0.7.5 (2026/06/25)

- [`renderView`](:) now accepts a ref callback
- Allow optional parameter matcher in [`registerView`](:)
- <kbd>fix</kbd> fire [`pageleave` event](/docs/router/WithRouter#s-pageleave-event) in nested view container before parent view is unmounted

## v0.7.4 (2026/06/21)

#### Views

- <kbd>fix</kbd> [`linkTo`](:) and [`navigateTo`](:) did not correctly resolve destination path from a matching route
  when the view are not registered with some optional parameter
- <kbd>fix</kbd> navigation halted when nested view container are unmounted during navigation

#### Dialog

- Add [`DialogContext.dialogElement`](:)
- <kbd>fix</kbd> container element of the dialog controller is detached before the last dialog finishes closing
  with outro animation
- <kbd>fix</kbd> type inference issue due to breaking change in react 19 <Badge.TS/>

## v0.7.3 (2026/06/07)

- <kbd>fix</kbd> reliably fire [`pageenter` event](/docs/router/WithRouter#s-pageenter-event) after component unmounted
- <kbd>fix</kbd> replace references of `JSX.Element` removed from React 19 in type definitions <Badge.TS/>

## v0.7.2 (2026/05/02)

- Add [`resetAllViews`](:) and [`ViewContext.setErrorView`](:) to reset rendered view
- Add [`useAppSessionState`](:) for managing app-level state persisted in session storage
- Add `signal` option for [`createDialog`](:) to dismiss dialog with `AbortSignal`

## v0.7.1 (2026/03/10)

#### Views

- [`registerView`](:) no longer cache view component from async factory
- <kbd>fix</kbd> error emitted to view context twice in development strict mode

## v0.7.0 (2026/02/09)

#### Dialogs

- Add [`openDialog`](:) as shorthand
- Allow more options for [`createDialogQueue`](:) as default for child dialogs

#### Mixins

- [`Mixin.use`](:) accepts class name string as first argument
- Remove signature of [`Mixin.use`](:) that accepts legacy ref as it has never worked <Badge.TS/>
- Type hint for props returned from [`Mixin.use`](:) <Badge.TS/>

## v0.6.7 (2025/01/07)

#### Dialogs

- Add `container` and `controller` option for [`createDialog`](:)
- Add [`createDialogQueue`](:)

#### Mixins

- Allow passing options to mixin through [`useMixin`](:)
- <kbd>fix</kbd> missing export for [`StaticAttributeMixin`](:)
