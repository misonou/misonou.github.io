...
- <kbd>fix</kbd> prevent throwing on reset when supplying nonexistent filter property to [`DataView.reset()`](:)

#### Utilities

- <kbd>fix</kbd> [`useDependency`](:) did not return value sent from provider in react 19 strict mode

## v0.6.5 (2026/07/23)

#### Forms

- Resetting form with new initial data using dependency list [[link]](/docs/react/form/useFormContext)
- Restoring from previous view state, e.g. when navigating back to previous page
- Add [`FormContext.disabled`](:) and [`FormFieldState.disabled`](:)
- <kbd>fix</kbd> inconsistent behavior when value changes during initial render
- <kbd>fix</kbd> [`FormFieldState.version`](:) should be updated when value is reset to a different value

#### Utilities

- Add [`AsyncContentState.staled`](:) property

## v0.6.4 (2026/07/13)

#### Forms

- <kbd>fix</kbd> unnamed field being reset after hot reload, bug introduced in 0.6.1
- <kbd>fix</kbd> [`dataChange` event](/docs/react/form/FormContext#s-datachange-event) fired after calling [`FormContext.reset`](:) when there were pending changes
- <kbd>fix</kbd> perform correct comparison on dependency list for hooks in [`FieldType.postHook`](:)

#### Utilities

- Accept dependency list to [`useAbortSignal`](:)
- <kbd>fix</kbd> type inference issue due to breaking change in react 19 <Badge.TS/>

## v0.6.3 (2026/06/07)

- <kbd>fix</kbd> [`useAsync`](:) did not refresh for data after hot reload in React 19
- <kbd>fix</kbd> singletons may be disposed early prior to unmount when a shared dispose callback is passed to [`useSingleton`](:) in development mode
- <kbd>fix</kbd> replace references of `JSX.Element` removed from React 19 in type definitions <Badge.TS/>

## v0.6.2 (2026/05/02)

- Add [`useAbortSignal`](:) and [`useSideEffect`](:)

## v0.6.1 (2026/03/03)

#### Forms

- [`FormFieldState.key`](:) now returns unique keys instead of empty string for fields with form context
- [`<Form>`](:) and [`<FormContextProvider>`](:) now throw errors when the supplied context object is not a [`FormContext`](:) instance
- <kbd>fix</kbd> [`FormFieldState.form`](:) should return `null` instead of the internal form context for fields without parent form context
- <kbd>fix</kbd> memory leak after unnamed field is mounted in development strict mode

## v0.6.0 (2026/02/03)

#### Forms

- Pass cancellation reasons to [`beforeLeave`](/docs/react/form/FormContext#s-beforeleave-event) event as `reason` property
- Focus input associated with nested data when the path passed to [`FormContext.focus`](:) does not have a direct field
- `memo` hook callbacks in [`FieldType.postHook`](:) are now invoked with values in the dependency list as arguments
- [`FormFieldProps.onValidate`](:) now works for field without form context, unless `validateOnChange` is set to `false`
- Field meta
  - add [`FormFieldState.meta`](:) for meta information about current value
  - add [`FieldType.getMeta`](:) for providing additional meta information
  - invoke [`FormFieldProps.onValidate`](:) with a forth argument from the [`FormFieldState.meta`](:) property
- [`ChoiceFieldProps.items`](:) can now be initially `undefined` without resetting value
- Relative dates on [`DateField`](:)
  - extended functions for relative date (see [Relative dates](/docs/react/form/DateField#s-relative-dates) for details)
  - no longer carry over to the following month if the target month has fewer days than the source date,
    e.g. `2024-01-31` plus one month will result in `2024-02-29` instead of `2024-03-02`

#### Utilities

- **Breaking change** &mdash; `undefined` and `null` is now ignored when passed to dependency provider, acting like if it was not mounted
- [`partial`](:) now returns stable callbacks
- <kbd>fix</kbd> [`partial`](:) should treat `NaN` as equal values and `0` and `-0` as different values

## v0.5.17 (2025/02/13)

- <kbd>fix</kbd> uninitialized variable access due to `setValue` being called synchronously in field type's hook

## v0.5.16 (2025/01/10)

- <kbd>fix</kbd> text input interfered when field is controlled

## v0.5.15 (2024/12/05)

#### Forms

- Add [`DateField.getDate`](:) for resolving relative dates
- Accept multiple values for [`MultiChoiceFieldState.toggleValue`](:)
- <kbd>fix</kbd> handle extended ISO date format correctly
- <kbd>fix</kbd> incorrectly reported unchanged prop in [`dataChange` event](/docs/react/form/FormContext#s-datachange-event)
- <kbd>fix</kbd> [`FormFieldProps.onChange`](:) callback fired during render for controlled field

#### Utilities

- Add `comparer` parameter for [`useObservableProperty`](:)
