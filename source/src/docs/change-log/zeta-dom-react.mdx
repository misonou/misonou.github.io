...
- Add support for data path hinting for [`FormContext`](:) methods
- Callback supplied to [`ErrorHandler`](:) receives a second argument `context`
- <kbd>fix</kbd> type definition issue which may break existing code <Badge.TS/>
  - corrected type of [`ZetaEventBase.target`](:) for [`FormContext`](:) and [`DataView`](:) events
  - updated type argument of [`useRefInitCallback`](:) for correct type inference [[link]](/docs/troubleshoot/typescript#s-updated-type-argument-of-userefinitcallback)
...
- <kbd>fix</kbd> variance of dependency type, particularly [`Dependency`](:) is now **bivariant**,
  whereas [`Dependency.Provider`](:) is now **contravariant** [[link]](/docs/react/createDependency#s-type-safety)
