...
- Add [`ZetaEventContainer.Target`](:)
- Improvement on observable property:
  - callback returned from [`defineObservableProperty`](:) allow second `receiver` argument
  - when inherited, it will be initialized if the given initial value is primitive
  - `0` and `-0` are now considered different values
- Invoke [`subscribeAsync`](:) callback immediately if async operation is already in progress

## v0.6.5 (2026/05/02)

- Add [`isObservableProperty`](:)
- Add [`throwIfAborted`](:)
- Accept error object as 2nd argument for [`errorWithCode`](:)

## v0.6.4 (2026/03/10)

- Add [`ZetaEventContainer.has`](:)
- <kbd>fix</kbd> callback passed to [`watch`](:) without specifying specific property
  - should be triggered when an alias property updates
  - should receive alias property's value on `oldValues` and `newValues` object when such property is aliased to
    another property on the same object
- <kbd>fix</kbd> type definitions for [`single`](:) and [`definePrototype`](:) <Badge.TS />

## v0.6.3 (2025/12/12)

- <kbd>fix</kbd> prevent scrolling non-focusable element when dragging

## v0.6.1 (2025/01/15)

- Fix several issues with `textInput` event:
  - `data` property now returns `\n` for `enter` key for practical use
  - event should not be triggered by `enter` key for `<input>` element
  - event was not fired when using certain IME in mac and iOS
- <kbd>fix</kbd> manually dispatched `click` event was not fired after mouse movement
- <kbd>fix</kbd> [`textInputAllowed`](:) returns `false` for `<select>` element and `<input>` element
  that does not allow text edits (for type where `selectionStart` and `selectionEnd` returns `null`)
- <kbd>fix</kbd> remove unreliable detection of input event source
