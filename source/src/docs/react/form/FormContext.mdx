...
<MemberList
    i="FormContext"
    ip={['isValid', 'data', 'ref', 'autoPersist', 'preventLeave', 'validateOnChange', 'enterKeyHint', 'formatError', 'disabled']}
...
### `FormContext.disabled` <Badge.Version />
...
It also requires field components to handle the disabled state exposed by [`FormFieldState.disabled`](:).
...
<dl>
    #### `error`
    [`ValidationError`](:) instance returned from validation callback.

    #### `name`
    Data path of the field that caused the validation error.

    #### `props`
    Properties being passed to [`useFormField`](:).

    #### `form`
    Associated `FormContext` instance if any.

</dl>

> Note that the callback will not be invoked for values other than [`ValidationError`](:).

### `FormContext.isValid` <Badge.ReadOnly />
...
### `FormContext.ref` <Badge.ReadOnly />

A React ref function to be passed to the `<form>` element.
...
Gets the form element or the input element for the specified field.
...
Focus the form element or the input element for the specified field.
...
<dl>
    #### `kind` <Badge.Optional /> <Badge.Version />
...
It is overridden by instance's `formatError` property.
...
<dl>
    #### `reason` <Badge.Version />
    Cancellation reason passed to [`cancelLock`](:).
</dl>
...
- **0.6.0** `beforeLeave` event: `reason` property
- **0.6.5** `FormContext.disabled`
</VersionTimeline>
