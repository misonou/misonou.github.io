import { MDXTableElement } from "src/components/docs/MDXTableElement";
...
## Object path
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{prop}}'}</code></td><td>Property or global variable</td></tr>
<tr><td width="200px"><code>{'{{obj.prop}}'}</code></td><td>Property of descendent object</td></tr>
<tr><td width="200px"><code>{'{{arr.0}}'}</code> <code>{'{{arr.1}}'}</code>...</td><td>First (and second ...) element in an array</td></tr>
<tr><td width="200px"><code>{'{{arr.^0}}'}</code> <code>{'{{arr.^1}}'}</code>...</td><td><kbd>v2.4</kbd> Last (and second last ...) element in an array</td></tr>
<tr><td width="200px"><code>{'{{obj.$(name)}}'}</code></td><td>Evaluated property name</td></tr>
</MDXTableElement>


## Special variable
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{@random}}'}</code></td><td><kbd>v2.4</kbd> A random number between 0 and 1</td></tr>
<tr><td width="200px"><code>{'{{@now}}'}</code></td><td><kbd>v2.4</kbd> Timestamp of current time</td></tr>
<tr><td width="200px"><code>{'{{@today}}'}</code></td><td><kbd>v2.4</kbd> Timestamp of today's date</td></tr>
<tr><td width="200px"><code>{'{{@global}}'}</code></td><td>Global object passed to waterpipe</td></tr>
<tr><td width="200px"><code>{'{{@0}}'}</code> <code>{'{{.}}'}</code></td><td>Current object or value</td></tr>
<tr><td width="200px"><code>{'{{@1}}'}</code> <code>{'{{@2}}'}</code>...</td><td>Object or value in outer scope</td></tr>
<tr><td width="200px"><code>{'{{@root}}'}</code> <code>{'{{_}}'}</code></td><td>Object or value passed to waterpipe</td></tr>
<tr><td width="200px"><code>{'{{#key}}'}</code> <code>{'{{#}}'}</code></td><td>Key of current iteration in <code>{'{{foreach}}'}</code></td></tr>
<tr><td width="200px"><code>{'{{#index}}'}</code> <code>{'{{##}}'}</code></td><td>Zero-based index of current iteration in <code>{'{{foreach}}'}</code></td></tr>
<tr><td width="200px"><code>{'{{#count}}'}</code></td><td>Total iteration count in <code>{'{{foreach}}'}</code></td></tr>
</MDXTableElement>

> **Note**
  Shorthand `.`, `_`, `#` and `##` must be in evaluated mode (`$.`, `$_`, `&#` and `&##`) when placing for argument.

## Statement
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{if '}<kbd>exp</kbd>{'}}'}</code><br/><code>{'{{if not '}<kbd>exp</kbd>{'}}'}</code></td><td>Logic branching<br/>Support <code>{'{{elseif}}'}</code> <code>{'{{elseif not}}'}</code> and <code>{'{{else}}'}</code></td></tr>
<tr><td width="200px"><code>{'{{foreach '}<kbd>exp</kbd>{'}}'}</code></td><td>Iterate over an object or array</td></tr>
<tr><td width="200px"><code>{'{{foreach}}'}</code></td><td>Iterate over the current object or array</td></tr>
<tr><td width="200px"><code>{'{{!'}<kbd>comment</kbd>{'}}'}</code></td><td>Comment</td></tr>
</MDXTableElement>


## Output
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{&html}}'}</code></td><td>Turn off HTML character escaping<br/><code>{'{{html}}'}</code> <kbd>&#x279c;</kbd> <code>&amp;lt;p&amp;gt;&amp;lt;/p&amp;gt;</code><br/><code>{'{{&html}}'}</code> <kbd>&#x279c;</kbd> <code>&lt;p&gt;&lt;/p&gt;</code></td></tr>
<tr><td width="200px"><code>{'{{objOrArray}}'}</code></td><td>Object is output as JSON<br/><code>{'{{&obj}}'}</code> <kbd>&#x279c;</kbd> <code>{'{"prop":true,"arr":[1,2,3]}'}</code></td></tr>
</MDXTableElement>


## Pipe
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{value + 1}}'}</code></td><td>Call <code>+</code> with argument <code>1</code> on input <code>value</code></td></tr>
<tr><td width="200px"><code>{'{{+ 1}}'}</code></td><td>Call pipe on current object or value in absence of name collision</td></tr>
<tr><td width="200px"><code>{'{{str substr 0 1 upper}}'}</code></td><td>Chaining pipe functions (left-to-right evaluation)</td></tr>
</MDXTableElement>

## Input resolution
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{[ value + 1 ] / 2}}'}</code></td><td>Pipe as input</td></tr>
<tr><td width="200px"><code>{'{{"constant" length}}'}</code></td><td>Constant as input</td></tr>
<tr><td width="200px"><code>{'{{$length length}}'}</code></td><td>Forced evaluation on input in case having name collision</td></tr>
</MDXTableElement>

## Argument resolution
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{== [ 1 / 2 ]}}'}</code></td><td>Pipe as argument</td></tr>
<tr><td width="200px"><code>{'{{arr map [ prop ]}}'}</code></td><td>Pipe as function argument</td></tr>
<tr><td width="200px"><code>{'{{== 1}}'}</code><br/><code>{'{{== true}}'}</code></td><td>Numbers, booleans, <code>undefined</code> and <code>null</code> are always as is</td></tr>
<tr><td width="200px"><code>{'{{== arg}}'}</code></td><td>Auto mode<br/> <kbd>=</kbd> <code>{'{{== $arg}}'}</code> if <code>arg</code> exists<br/> <kbd>=</kbd> <code>{'{{== "arg"}}'}</code> if <code>arg</code> does not exist</td></tr>
<tr><td width="200px"><code>{'{{== $arg}}'}</code><br/><code>{'{{== $1}}'}</code></td><td>Forced evaluation</td></tr>
<tr><td width="200px"><code>{'{{== "arg"}}'}</code></td><td>Forced constant value</td></tr>
<tr><td width="200px"><code>{'{{value1 == value2}}'}</code><br/><code>{'{{object1 == value2}}'}</code><br/><code>{'{{object1 == object2}}'}</code></td><td>Permitted evaluation in auto mode</td></tr>
<tr><td width="200px"><code>{'{{value1 == object2}}'}</code></td><td>Fallback to forced constant value<br/> <kbd>=</kbd> <code>{'{{value1 == "object2"}}'}</code></td></tr>
</MDXTableElement>


## Special functions
<MDXTableElement style={{ minWidth: "500px", tableLayout: "fixed" }}>
<tr><td width="200px"><code>{'{{arr1 | arr2}}'}</code></td><td>Concatenates arrays or values into new array</td></tr>
<tr><td width="200px"><code>{'{{e1 && e2}}'}</code><br/><code>{'{{e1 || e2}}'}</code><br/><code>{'{{e1 && [ e2 || e3 ]}}'}</code></td><td>Boolean logic</td></tr>
<tr><td width="200px"><code>{'{{let name value}}'}</code></td><td>Save <code>value</code> to global variable named <code>name</code></td></tr>
<tr><td width="200px"><code>{'{{value as name}}'}</code></td><td>Save <code>value</code> to global variable named <code>name</code> and output the value</td></tr>
</MDXTableElement>


## Built-in functions

<MDXTableElement style={{ whiteSpace: "nowrap" }}>
<tr>
...
<tr>
    ...
    <td width="425px">
        <code>{'{{"foo" & bar}}'}</code> <kbd>&#x279c;</kbd> <code>foobar</code> <span>(alias in <kbd>v2.4</kbd>)</span><br/>
...
<tr>
    ...
    <td width="425px">
        ...
        <code>{'{{0 :date yyyy-MM-dd}}'}</code> <kbd>&#x279c;</kbd> <code>1970-01-01</code><br/>
        <code>{'{{&:json}}'}</code> <kbd>&#x279c;</kbd> <code>{'{"p1":"value1","p2":"value2"}'}</code>
    </td>
</tr>
<tr>
...
<tr>
    <td width="125px" valign="top"><i>Evaluation</i></td>
    <td width="175px">
        <code>|&gt;</code> <code>eval</code> <kbd>v2.8</kbd><br/>
        <code>select</code> <kbd>v2.10</kbd>
    </td>
    <td width="425px">
        <code>{'{{"foo" |> template}}'}</code> <kbd>&#x279c;</kbd> <code>Value is foo</code><br/>
        <code>{'{{obj select [ prop ]}}'}</code> <kbd>&#x279c;</kbd> <code>Got obj.prop</code>
    </td>
</tr>
</MDXTableElement>
