...
<dl>
    ...
    <dl>
        ...
        #### `closeOnNavigate` <Badge.Optional /> <Badge.Version />
    ...
    > The `closeIfOpened` parameter is deprecated. Use [`toggleFlyout`](:) instead.
...
<dl>
    ...
    #### `visible`
    When flyout is open, the `visible` CSS class will be added.
</dl>

> After flyout is completely hidden, both `open`, `closing` and `visible` CSS classes will be removed.
...
- **0.7.1** `closeOnNavigate` `popover`  `preventLeave` `preventNavigation`option
</VersionTimeline>
