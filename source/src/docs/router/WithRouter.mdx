...
<MemberList
    i="app" noRoot
    ip={['path', 'basePath', 'initialPath', 'route', 'routes', 'canNavigateBack', 'canNavigateForward', 'previousPath', 'sessionId', 'sessionStorage', 'cache', 'page', 'navigationType', 'historyStorage']}
    im={['navigate()', 'back()', 'backToPreviousPath()', 'snapshot()', 'resolvePath()', 'parseRoute()', 'matchRoute()', 'isAppPath()', 'toHref()', 'fromHref()']}
    ev={['beforepageload', 'navigate', 'pageload', 'hashchange', 'pageenter', 'pageleave', 'popstate', 'pushstate']}/>
...
Returns a [`HistoryStorageContainer`](:) instance for accessing storage associated to history states.

### `app.initialPath` <Badge.ReadOnly />

Gets the initial path before the app is initialized.

### `app.navigationType` <Badge.ReadOnly /> <Badge.Version />

Gets how the current page is navigated. It has the following possible values:
...
### `hashchange` event <Badge.Version />
...
### `popstate` event <Badge.Version />

See [`popstate` event](/docs/router/popstate-event).

### `pushstate` event <Badge.Version />
...
- **0.6.16** `app.navigationType`<br/>`pushstate` event
</VersionTimeline>
