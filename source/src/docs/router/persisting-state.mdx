...
> **Important**
  Session storage has a typical 5MB limitation. Although data is compressed, storing huge data should be avoided.


> **Important**
  Although session storage is scoped to the document's origin and is cleared when the tab is closed, do not store critically sensitive data.
